...
template<typename Real>
void ccopy ( int n, Real x[], Real y[] );
template<typename Real>
...
int main ( void )
{
  ...
  if(main_fft< double >("double") != 0)
    return 1;
  // Float - FP32
  if(main_fft< float >("float") != 0)
    return 1;
  // Posit<32,2>
  if(main_fft< posit<32,2> >("posit<32,2>") != 0)
    return 1;
  // Posit<16,2>
  if(main_fft< posit<16,2> >("posit<16,2>") != 0)
    return 1;
  // Posit<16,1>
  if(main_fft< posit<16,1> >("posit<16,1>") != 0)
    return 1;
...
template<typename Real>
void step ( int n, int mj, Real a[], Real b[], Real c[],
  Real d[], Real w[], double sgn )
