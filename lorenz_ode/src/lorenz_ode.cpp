...
template<typename Real>
Real *r8vec_linspace_new ( int n, Real a, Real b );
template<typename Real>
...
int main ()
{
  ...
  if(main_lorenz< posit<16,2> >("posit_16_2") != 0)
    return 1;
  // Posit<16,1>
  if(main_lorenz< posit<16,1> >("posit_16_1") != 0)
    return 1;
...
//
  t = r8vec_linspace_new ( n + 1, ( Real )(0.0), t_final );
  x = new Real[m*(n+1)];
  x[0+0*m] = 8.0;
  x[0+1*m] = 1.0;
  x[0+2*m] = 1.0;
...
template<typename Real>
Real *r8vec_linspace_new ( int n, Real a_first, Real a_last )
